...
from .database_scripts import check_unique_name
...
def nameAPI(request):
    if request.method == 'GET':
        ...
        if names_serializer.is_valid():
            q = queue.Queue()
            thread = threading.Thread(target = check_unique_name, args = [names_serializer.validated_data['name'], q])
            thread.start()
            thread.join()
            if q.get():
                names_serializer.save()
                return JsonResponse('POST successful', safe = False, status = 200)
            else:
                return JsonResponse('This name already exists', safe = False, status = 201)
        return JsonResponse('POST failed', safe = False, status = 500)
